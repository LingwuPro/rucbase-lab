...
class DeleteLogRecord : public LogRecord {
   public:
    ...
    void format_print() override {
        ...
        char table_name[table_name_size_ + 1];
        memcpy(table_name, table_name_, table_name_size_);
        table_name[table_name_size_] = '\0';
        printf("table name: %s\n", table_name);
    }
...
class UpdateLogRecord : public LogRecord {
   public:
    ...
    RmRecord before_value_;   // 修改前的记录
    RmRecord after_value_;    // 修改后的记录
    Rid rid_;                 // 记录插入的位置
    char* table_name_;        // 插入记录的表名称
    size_t table_name_size_;  // 表名称的大小
};
